...
Quando il pedone finisce di attraversare il contatore corrispondente viene decrementato di 1, e se
$x_{dir} \neq x_{side} \land x_{side} \neq \textit{straight}$,
...
Tutte le auto in $\textit{Arrival}(i)$ che possono passare insieme vengono aggiunte a $\textit{Crossing}(i)$ e rimosse una volta lasciato l'intersezione.
Finché $\textit{Crossing}(i)$ è piena non potranno essere aggiunte altre auto.

%
Se ci sono pedoni che stanno attraversando le strade $(x_{curr}, x_{next})$ e $(x_{prev}, x_{curr})$ l'auto dovrà attendere
e passare quando $C(x_{curr},x_{prev})$ = 0 e $C(x_{curr},x_{next})$ = 0 (Fig. \ref{fig:auto-ped-crossing}).
